...
class Contact {
    ...
    static async list_elt(filters = {}) {
        try {
            // Applique les filtres fournis dans la requête 'findAll'
            const entries = await ContactModel.findAll({
                where: filters
            });
            ...
            // Transforme les entrées en format JSON
            return entries.map(entry => Contact.fromJson(entry.toJSON()).toJson());
        } catch (error) {
            throw error;
        }
...
// async delete() {
//     try {
//         const existingEntry = await ContactModel.findOne({where: {guid: this.guid}});
//         await W.isOccur(!existingEntry, W.errorGuid);
//
//         const deleted = await ContactModel.destroy({where: {guid: this.guid}});
//         await W.isOccur(deleted === 0, W.errorDeleted);
//     } catch (error) {
//         throw error;
//     }
